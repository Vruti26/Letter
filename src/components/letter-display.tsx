...
export function LetterDisplay({ letterContent, userName }: { letterContent: string; userName: string }) {
  ...
  return (
    <div className="w-full max-w-2xl">
      ...
      >
        <div className="absolute inset-0 bg-secondary rounded-lg shadow-2xl shadow-primary/20"></div>
        {/* Flap */}
        <div
          ref={flapRef}
        ...
        <div className="absolute inset-0">
          <div className="absolute bottom-0 left-0 w-full h-1/2 bg-secondary" style={{ clipPath: 'polygon(0 100%, 100% 100%, 50% 0)' }}></div>
          <div className="absolute top-0 left-0 w-1/2 h-full bg-secondary/80" style={{ clipPath: 'polygon(0 0, 100% 50%, 0 100%)' }}></div>
          <div className="absolute top-0 right-0 w-1/2 h-full bg-secondary/80" style={{ clipPath: 'polygon(100% 0, 0 50%, 100% 100%)' }}></div>
        </div>
        <div className="absolute inset-0 flex items-center justify-center">
             <p className="text-lg text-primary-foreground font-semibold opacity-80 animate-pulse">Click to open</p>
        </div>
      </div>
